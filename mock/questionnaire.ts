...
const getQuests = (req: Request, res: Response) => {
  questionnaires.map((quest) => {
    if (quest.questionnaire.identifier !== req.params.identifier) {
      return res.status(401).send({
        errorCode: '401',
        errorMessage: 'Unauthorized',
      });
    }
    return res.json(quest.questionnaire);
  });
};
...
export default {
  // GET request mock
  'GET /api/questionnaire/:identifier': getQuests,
};
